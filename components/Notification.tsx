...
export default function Notification(props: any) {
  ...
  return (
    <div
      onClick={() => notify()}
      className={`flex cursor-pointer  flex-row px-8 py-5 space-x-5 border-b border-b-neutral-900 ${!props.seen ? "bg-cyan-500/10" : "bg-black"}`}
    >
      ...
      <div className="flex flex-col space-y-[2px]">
        ...
        <h1 className="text-xs text-neutral-500 lg:text-sm">{props.description}</h1>
      </div>
    </div>
  );
}
