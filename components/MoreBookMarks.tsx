...
const date1 = new Date();
    ...
    async function get(from,to){
        ...
        let l = u[0]["bookmarks"]
            const {data,error} = await supabase.from('posts').select('*').order('id',{ascending:false}).in('id',l).textSearch('title_excerpt_content', `'${props.slug}' | '${props.slug.toLowerCase()}' | '${props.slug.toUpperCase()}'`).range(from,to)
            if(error){
              console.log(error)
            }
            else{
