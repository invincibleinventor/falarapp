...
export default function UserInformation(props: any) {
  ...
  const handleMouseLeave = (e: React.MouseEvent) => {
    ...
    if (rect && popupRect) {
      ...
      const isBetweenElements =
        (cursorY > rect.bottom && cursorY < popupRect.top) || (cursorY < rect.top && cursorY > popupRect.bottom);
  ...
  useEffect(() => {
    function updatePosition() {
      ...
      const newPosition = {
        top: showAbove ? rect.top + window.scrollY - popupHeight + 20 : rect.bottom + window.scrollY + 10,
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (u?.user) {
        ...
        const { data: mine, error: e } = await supabase
          .from("user")
          .select("image,handle,blocked,blockedby")
          .eq("id", u.user.id);
        ...
        if (data && mine && !error && mine.length > 0 && data.length > 0) {
          ...
          if (props.imfollowing !== undefined) {
            ...
            if (data[0].followers.includes(mine[0].handle)) setImFollowing(true);
            else setImFollowing(false);
          }
        }
      }
    }
          ...
          <div className="flex flex-col mx-2 mt-8 space-y-2">
            ...
            <div className="grid grid-cols-2 pt-2">
              <div className="flex flex-row content-center items-center space-x-1">
                <h1 className="text-sm font-semibold text-neutral-300">{user.followers?.length}</h1>
                <h1 className="text-sm text-neutral-400">Followers</h1>
              </div>
              <div className="flex flex-row content-center items-center space-x-1">
                <h1 className="text-sm font-semibold text-neutral-300">{user.following?.length}</h1>
