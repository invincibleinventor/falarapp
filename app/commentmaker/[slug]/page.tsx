...
export default function Create({ params }: { params: Promise<{ slug: string }>}) {
    ...
    return new Promise<void>((resolve, reject) => {
      const upload = new tus.Upload(file, {
        ...
        metadata: {
          ...
          objectName: "comments/images/" + id + "/" + fileName,
                ...
                for (let i = 0; i < imgsSrc.length; i++) {
                  ...
                  uploadFile(bucket, id, i + ".jpg", file).then(async () => {
                    ...
                    for (let j = 0; j < imgsSrc.length; j++) {
                      imgarray.push(
                        "https://xiexuntwvmedvyxokvvf.supabase.co/storage/v1/object/public/quickies/comments/images/" +
                          id +
                          "/" +
                          j +
                          ".jpg"
  ...
  return (
    <div className="flex  h-screen w-[calc(100vw-68px)] bg-neutral-900/20 flex-col content-center items-start md:w-full">
      ...
      <textarea
        ...
        maxLength={150}
        className="px-6 py-5 mb-auto w-full h-full bg-transparent outline-none resize-none text-neutral-300 text-md md:text-lg placeholder:text-neutral-600 md:m-4 md:p-0 md:px-2"
        placeholder="What's on your mind?"
      ></textarea>
      <div className="grid grid-cols-3 px-4 mb-20 w-full border-t bg-black/40 border-t-neutral-900 sm:flex sm:flex-row">
      ...
      <div className="flex absolute bottom-0 flex-row content-center items-center px-4 pl-4 w-full h-20 bg-black border-t border-t-neutral-900">
        <input
          ref={hiddenFileInput}
          type="file"
          className="hidden"
        ...
        <button
          disabled={disabled}
          onClick={() => publish()}
          className="px-6 py-3 ml-auto text-xs font-medium text-white bg-blue-700 rounded-full md:mr-6 lg:mr-2"
        >
          Publish
        </button>
      </div>
