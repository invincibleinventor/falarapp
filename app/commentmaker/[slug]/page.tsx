...
const formatText = (text:string) => {
  ...
  async function publish(){
    ...
    if(!error && data){
      ...
      const {error}  = await supabase.from('commentquickies').insert({id:params.slug,handle:handle,content:text})
      if(error){
        console.log('initial')
        console.log(error)
        ...
        const {data,error}  = await supabase.from('commentquickies').select('*').eq("poster", user?.user.id)
          .order("comment_id", { ascending: false })
          .limit(1);
                ...
                if(error){
                  ...
                  if(data && data.length>0){
                    ...
                    a={}
                     ...
                     const {error} = await supabase.from('trending').insert({date:date,[h]:a})
                     if(!error)
                       window.location.replace('/quicki/e'+params.slug)
                     }
                    }
            ...
            for(let i =0;i<imgsSrc.length;i++){
              ...
              uploadFile(bucket,id,i + ".jpg",file).then(async()=>{
                ...
                for(let j = 0;j<imgsSrc.length;j++){
                  imgarray.push("https://xiexuntwvmedvyxokvvf.supabase.co/storage/v1/object/public/quickies/comments/images/"+id+'/'+j+'.jpg')
                }
                const {error} = await supabase.from('commentquickies').update({image:imgarray}).eq('comment_id',id)
              if(error){
                console.log(error)
