...
useEffect(()=>{
  async function get(){
    ...
    const {data,error} = await supabase.from('posts').select('*').order('id',{ascending:false}).textSearch('title_excerpt_content', `'${search}' | '${search.toLowerCase()}' | '${search.toUpperCase()}'`).in('id',l).limit(5)
    if(error){
      console.log(error)
    }
    else{
...
<PostComponent id={post.id} title={post.title} time={timeAgo.format(Date.now() - post.diff)} key={post.id} image={post.image} dp={post.dp} handle={post.handle} name={post.name} description={post.excerpt}/>
    ...
    (
      <div className="flex items-center content-center w-full px-10 mt-24 lg:px-24 sm:px-24 md:px-16">
            ...
            <h1 className="mx-auto text-sm text-center text-neutral-400">Your bookmarked posts appear in here. Seems like you have not bookmarked anything that matches your search. Bookmark posts to view them here.</h1>
...
<Oval
   ...
   />
</div>)}<MoreBookMarks slug={search}/> </div> </div></div>
