...
export default function Toggle(props:any){
    ...
    async function deactivate(){
        ...
        async function moveRecord(sourceTable: string, destinationTable: string, recordId: string,handle:string): Promise<void> {
            setDisabled(true)
    ...
    if(handle)
        moveRecord('user', 'deactivated',user.user.id,handle[0]["handle"]);
    }
        
    }
    return(
        <button disabled={disabled} onClick={()=>deactivate()} className="px-6 py-3 text-xs font-medium text-white bg-red-800 rounded-full w-max" >Delete Account</button>
    )
}