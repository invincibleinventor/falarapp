...
get()},[])
  ...
  <div className="p-4 py-2 pb-4 mx-1 md:mx-1">
    ...
    <input className="w-full border border-gray-150 bg-white peer rounded-lg h-12 focus:outline-black focus:bg-white placeholder:text-neutral-600 font-poppins pl-14 pr-8 text-[14px]" placeholder="Search Evolt"></input>
    </div>
</div>
...
<PostComponent id={post.id} title={post.title} time={timeAgo.format(Date.now() - post.diff)} key={post.id} image={post.image} dp={post.dp} handle={post.handle} name={post.name} description={post.excerpt}/>
    ...
    (
      <div className="flex items-center content-center w-full px-10 mt-24 lg:px-24 sm:px-24 md:px-16">
            ...
            <Link href="/explore" className={`w-max mx-auto rounded-full text-xs font-bold mt-3 px-8 py-3 shadow-lg ${(1==1)?'bg-black text-white':'bg-white border-2'}`}>Explore People</Link>
