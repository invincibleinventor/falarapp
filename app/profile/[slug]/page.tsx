 ...
 {               setFound(true)
  ...
  <div className='flex-1 h-screen p-0 overflow-x-hidden overflow-y-hidden'>
        ...
        <div className="relative h-64">
            <div className="bg-red-200 h-48 w-[calc(100%)-8px]  m-4 rounded-md shadow-lg">
                {cover &&
        ...
        {found && <button onClick={()=>onfollow()}className={`absolute rounded-lg text-xs font-bold bottom-10 md:right-12 right-6 px-8 py-3 shadow-md ${!(imfollowing || myself)?'bg-red-400 text-white border-1 border-red-400':'bg-white text-red-500 border-2 border-red-400'}`}>{myself?'Edit Profile':imfollowing?'Unfollow':'Follow'}</button>
}
        </div>
