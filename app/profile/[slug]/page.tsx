    ...
    if (blocked) {
      ...
      if (error) {
        ...
        if (localfollowerlist.includes(myId)) {
          ...
          const { data, error } = await supabase.from("user").update({ followers: arr }).eq("handle", slug).select();
      ...
      <div className={`overflow-y-scroll pb-10 h-full hiddenscroll`}>
        ...
        {found && !blocked && !imblockedby && (
          <>
            ...
            {!blocked && !imblockedby && opened == "quickies" && <InduvidualQuickies handle={slug} />}

            {!blocked && !imblockedby && opened == "media" && <MediaGrid handle={slug} />}
          </>
        )}
      </div>
    </div>
