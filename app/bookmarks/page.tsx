...
let l=[]
  async function get(){
    ...
    const {data,error} = await supabase.from('posts').select('*').order('id',{ascending:false}).in('id',l).limit(5)
    if(error){
    }
    else{
...
await get()
  ...
  <div className="p-4 py-2 pb-4 mx-1 md:mx-1">
    ...
    <input className="w-full h-12 pr-8 text-[14px] bg-white border border-gray-150 peer focus:outline-black focus:bg-white placeholder:text-neutral-600 font-poppins pl-14" placeholder="Search Evolt"></input>
    </div>
</div>
...
<PostComponent id={post.id} cover={post.cover} title={post.title} time={timeAgo.format(Date.now() - post.diff)} key={post.id} image={post.image} dp={post.dp} handle={post.handle} name={post.name} description={post.excerpt}/>
    ...
    (
      <div className="flex items-center content-center w-full px-10 mt-24 lg:px-24 sm:px-24 md:px-16">
            ...
            <h1 className="mx-auto text-sm text-center text-neutral-400">Your bookmarked posts appear in here. Seems like you have not bookmarked anything yet. Bookmark posts to view them here.</h1>
