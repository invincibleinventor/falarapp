...
export default async function App({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return !loading ? (
    <div
      className={`flex overflow-hidden relative flex-col flex-1 h-screen bg-transparent ${!loggedin ? "lg:mx-96 border-x border-x-neutral-800" : ""}`}
    >
      {!loggedin && (
      ...
      {!error && !newblocked.includes(authorid) && !blocked.includes(authorid) && (
        <div className="hiddenscroll h-full w-[calc(100vw-68px)] mx-0 overflow-hidden pb-14 md:w-full md:max-w-full px-0">
          <div
            className={`flex fixed flex-row content-center items-center px-4 py-4 space-x-4 w-full text-white border-b shadow-lg z-[1000000]   ${AppConfig.customtheme ? `bg-${AppConfig.customcolor} border-b=${AppConfig.customcolor}` : "border-b-primary-950 bg-primary-950"}`}
          >
            <Back></Back>
            <h1 className="text-lg font-medium text-white font-poppins">Quickie</h1>
          </div>
              ...
              <div className="flex h-max flex-col gap-[8px] pt-4 ">
                {parentQuickie && (
                  ...
                  >
                    ...
                    <span className="block font-medium text-primary-500">Replying to @{parentQuickie.user.handle}</span>
                  </Link>
                )}
                <div className="flex flex-row gap-2 content-center items-center h-max shrink-0">
                  ...
                  {!parentQuickie && <Menu type="quickie" id={slug} myhandle={myhandle} handle={author} />}{" "}
                </div>
