...
get()},[])
  ...
  <div className="p-4 py-2 pb-4 mx-1 bg-opacity-50 md:mx-1 ">
    ...
    <input className="w-full border border-gray-150 bg-white peer h-12 focus:outline-black focus:bg-white placeholder:text-neutral-600 font-poppins pl-14 pr-8 text-[14px]" placeholder={"Search "+params.slug+"'s"+" posts"}></input>
    </div>
</div>
...
<PostComponent id={post.id} title={post.title} cover={post.cover} time={timeAgo.format(Date.now() - post.diff)} key={post.id} image={post.image} dp={post.dp} handle={post.handle} name={post.name} description={post.excerpt}/>
